...
public interface SqlConnectionModifier {
	...
	SqlConnectionModifier READ_ONLY =
			connection -> {
				try {
	...
	SqlConnectionModifier READ_WRITE =
			connection -> {
				try {
	...
	SqlConnectionModifier AUTO_COMMIT =
			connection -> {
				try {
	...
	SqlConnectionModifier MANUAL_COMMIT =
			connection -> {
				try {
	...
	SqlConnectionModifier TRANSACTION =
			connection -> {
				try {
					connection.setAutoCommit(false);
				} catch (SQLException e) {
	...
	SqlConnectionModifier TRANSACTION_READ_ONLY =
			connection -> {
				try {
					connection.setAutoCommit(false);
					connection.setReadOnly(true);
	...
	SqlConnectionModifier TRANSACTION_MAX_ISOLATION =
			connection -> {
				try {
	...
	SqlConnectionModifier TRANSACTION_READ_COMMITED =
			connection -> {
				try {
	...
	SqlConnectionModifier TRANSACTION_REPEATABLE_READ =
			connection -> {
				try {
